...
/*
 * 학우 - 관심사 기반 사람 찾기(사용자 지시: "자신의 목적에 맞는 사람을 찾는 식",
 * "너비가 긴 직사각형으로 여러명을 띄우는게 아니라, 크게크게 위아래 길이가 길고
 * 너비가 상대적으로 작은 직사각형으로 프로필사진과 그 사람의 관심사를 띄우는거야").
 *
 * 2026-09-08 /explore가 허브(app/explore/page.tsx)로 바뀌면서 이 화면이
 * "학우" 노드의 목적지로 이전됐다(app/explore/page.tsx 참고). 로직은 그대로
 * 옮겼고, 어두운 우주 테마 -> societies/certifications와 같은 밝은 종이
 * 테마로 겉모습만 바꿨다(paper-* 토큰, .paper-surface 스코프).
 *
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4" aria-hidden>
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="aspect-[3/4] animate-pulse rounded-xl border border-paper-line bg-paper-soft" />
      ))}
    </div>
  );
}
...
function UserCard({
  ...
  return (
    <Link
      ...
      className="flex aspect-[3/4] flex-col items-center overflow-hidden rounded-xl border border-paper-line/60 bg-paper-soft/70 px-3 py-5 no-underline shadow-[0_1px_2px_rgba(28,28,45,0.04),0_6px_20px_-8px_rgba(28,28,45,0.08)] transition-shadow hover:shadow-[0_2px_4px_rgba(28,28,45,0.05),0_10px_28px_-8px_rgba(28,28,45,0.10)]"
    >
      <span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border border-paper-line bg-paper text-[30px] md:h-20 md:w-20 md:text-[34px]">
        {user.avatarEmoji ?? "🔭"}
      </span>
      {/* shrink-0: 내용이 넘칠 때 flex가 텍스트부터 압축해 이름이 사라지는
          것 방지(검수 1번). */}
      <span className="mt-3 w-full shrink-0 truncate text-center font-sans text-body-sm font-semibold text-paper-ink">
        {user.displayName ?? "이름 없는 관측자"}
      </span>
      {user.bio && (
        <span className="mt-1 line-clamp-2 w-full shrink-0 text-center text-caption leading-snug text-paper-lo">
          {user.bio}
        </span>
      )}
      {/* 관심사 칩 - 발행 별자리 요소 빈도 상위. 공통 태그는 lit 강조(단, 종이
          지면 위에서는 certifications의 VerifiedBadge처럼 텍스트는 paper-ink로
          -  연한 별빛색(lit)을 글자색으로 쓰면 밝은 지면 위에서 대비가 안 나온다). */}
      {shownTags.length > 0 && (
        <span className="mt-auto flex w-full flex-wrap justify-center gap-1 pt-3">
          {shownTags.map((tag) => (
            <span
              ...
              className={
                ...
                (common.has(tag)
                  ? "border-lit bg-lit/25 font-medium text-paper-ink"
                  : "border-paper-line text-paper-lo")
              }
            >
              {tag}
            </span>
          ...
          {hiddenCount > 0 && (
            <span className="rounded-full border border-paper-line px-2 py-0.5 font-mono text-micro text-paper-lo">
              +{hiddenCount}
            </span>
          )}
        </span>
...
export default function ExplorePeoplePage() {
  ...
  useEffect(() => {
    ...
    router.replace(`/login?next=${encodeURIComponent("/explore/people")}`);
  ...
  return (
    <div className="paper-surface mx-auto max-w-4xl rounded-3xl border border-paper-line/60 bg-paper px-4 py-10 shadow-[0_2px_8px_rgba(20,20,40,0.06),0_24px_56px_-28px_rgba(20,20,40,0.28)] md:px-10 md:py-12">
      <Link
        href="/explore"
      ...
      <header className="mb-6 flex flex-col gap-1.5">
        <h1 className="font-serif text-display font-bold text-paper-ink">학우</h1>
        <p className="text-body-sm text-paper-lo">같은 별을 보는 사람을 찾아보세요</p>
      </header>

      {/* 검색창 - @로 시작하면 아이디 검색, 아니면 관심사·소개 검색(문법 힌트는
          placeholder + 아래 헬퍼 텍스트, 모드는 입력창 옆 배지로 가시화). */}
      <label className="flex items-center gap-2.5 rounded-lg border border-paper-line bg-paper-soft px-3.5 py-2.5 focus-within:border-paper-ink">
        <SearchIcon size={17} className="shrink-0 text-paper-lo" />
        <input
          type="search"
          value={query}
          ...
          className="w-full bg-transparent font-sans text-body-sm text-paper-ink placeholder:text-paper-lo focus:outline-none"
        />
        {trimmedQuery && (
          <span
            className={
      ...
      <p className="mb-7 mt-1.5 pl-1 text-micro text-paper-lo">
        이름·소개·관심사로 찾고, <span className="text-paper-ink">@</span>를 붙이면 아이디로 찾아요
      </p>
