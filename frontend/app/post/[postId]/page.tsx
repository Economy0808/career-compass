 ...
 * 열람은 로그인 필수(SNS 열람 모델 전환 - GET /api/posts/{id}가 익명 401) -
 * PostDetail이 401은 로그인 유도, 없는 글은 404 빈 상태를 각각 렌더한다.
 * 작성자 줄은 상세가 ownerId를 알려준 뒤 프로필을 이어서 불러온다(1회, 로그인
 * 상태에서만 상세가 채워지므로 이 fetch도 자연히 로그인 후에만 일어난다).
 */
