...
| `18dc18e` (백엔드) | 소스 게이트 bool `kogl_type` 거부 (51f19eb nit) | **PASS** | `isinstance(kogl_type, bool) or not isinstance(kogl_type, int)` → 거부, `test_bool_kogl_type_rejected`(표시·LLM 모두 거부). 레지스트리 오타(`yes`/`true`)로 인한 우발 허용 경로 차단. |
| `54f04cd` (백엔드) | Phase 1 진로 추론 스크립트(`scripts/infer_career_paths.py`) | **PASS** | course_catalog 읽기 전용(Firestore set/update/delete/batch 호출 0, `course_repo` 조회 함수만), 자격증명·env 변경 0(`FIRESTORE_EMULATOR_HOST`는 출력용 읽기만), 출력은 `app/etl/seeds/career_paths_draft.json`뿐. 실행은 `firebase emulators:exec --import` 안에서(프로덕션 도달 0, export 미사용). 입력은 공개 강의 데이터라 PII 없음. |
| `56109f0` (백엔드) | 진로 통합 스크립트 + 시드 JSON 2개(draft 135/consolidated 81) | **PASS + Phase 3 조건** | Firestore·네트워크 쓰기 0(로컬 JSON in/out), 시드에 비밀·PII 패턴 0, `reviewed: false`. 학과·분야·자격증힌트는 코드가 원본에서 합산(LLM은 name/description/primary_field/variants만). **조건**: Phase 3 로더는 `reviewed: true`가 아니면 적재 거부(LLM 생성 name/description은 표시 텍스트 — 사람 검수 필수), 진로에 URL류 필드가 생기면 서버권위 규칙 동일 적용. |
| `1b50d5b` (백엔드) | Phase 3 자격증 로더+검색 API, Q-Net 파싱/키 수정 | **PASS** | 로더 `assert_reviewed`(`is True`만 통과), 기본 dry-run·`--commit` 필수(Phase 3 조건 충족). 공개 검색의 `verified`는 컬렉션 기준 계산(큐레이션 True / 유저 제보 False 고정), 키는 settings에서만. |
...
### 부록: 라이브 finding L-1 — Cloud Run 소스 업로드에 backend/.env 포함 (2026-09-08 실측)
- **원인**: `318b143`이 추가한 `backend/.gcloudignore`에 `#!include:.gitignore`도 `.env` 제외도 없음. `.gcloudignore`가 있으면 gcloud는 `.gitignore`를 더 이상 참조하지 않아 `--source .` 배포 시 `.env`가 소스 zip에 포함됨.
- **실측**: `gs://run-sources-ourlab-0808-asia-northeast3/services/ourlab-backend/1788771545.77198-5f91ea3a04f9425ea64da51e700a2f19.zip`(2026-09-07 17:59 KST, 커밋 이후 유일한 백엔드 업로드) 내부에 `.env` 존재(zip 목록 확인, 로컬 사본 즉시 삭제). 이전 업로드 3건은 커밋 전(.gitignore 적용). 프론트는 `.gcloudignore` 없음 → 무관.
- **범위**: Dockerfile은 app/alembic/pyproject만 COPY → **이미지 깨끗**. 노출 위치는 프로젝트 프라이빗 버킷 객체 1개(프로젝트 principal만 열람). 외부 노출 아님. 심각도 **Medium**(Secret Manager 밖 영속 아티팩트의 실키).
- **포함된 키(이름만)**: ANTHROPIC_API_KEY(108자, 실키·Secret Manager와 동일 추정) / RESEND_API_KEY(re_ 실키) / OPENAI_API_KEY(164자 실키, 코드 미사용) / DATA_GO_KR_API_KEY·SERVICE_KEY(실키, 신규 활용신청 시 로테이션 예정이던 것) / SECRET_KEY(36자) / SOLAPI(3자 플레이스홀더) / DATABASE_URL(로컬 postgres). **페퍼 판정**: 로컬 .env의 SECRET_KEY(36자)는 운영 페퍼(Secret Manager secret-key, 64자)와 다른 값 → 학번 해시 페퍼 미노출, 페퍼 로테이션 불필요(로테이션하면 기존 학생 해시 전부 무효).
- **조치**: ① `backend/.gcloudignore`에 `#!include:.gitignore` + `.env`, `.env.*`, `*.pem`, `var/` 명시 제외(백엔드 세션, 재게이트 대상) ② 해당 zip 객체 삭제(사용자 승인) ③ 키 로테이션 — 사용자 결정: Anthropic(Secret Manager 동시 갱신)·Resend·OpenAI(폐기)·data.go.kr(신규 신청과 묶음). ④ 다음 배포 뒤 최신 zip 재검사로 종결 확인.
| `7c48316`·`97f7a43`·`bf0e042`·`9a6ae96`·`709a0f6`·`fe92334` (프론트, 일괄) | 학회/자격증 페이지 camelCase 수정·재작성·스타일·내비 | **PASS** | 현재 파일 상태 실측: 양 페이지 앵커는 `safeLinkHref` 통과분만(`noopener noreferrer`), 배지 `verified` 분기 유지, 학회 폼 필드 이름·공식 링크·모집 시기·세부 분야·설명(연락처 없음)+안내 문구 유지, dangerouslySetInnerHTML·innerHTML·localStorage·sessionStorage 0. |
| (커밋 아님) dongari 학회 594건 프로덕션 벌크 적재 (백엔드, 2026-09-07, 사용자 지시) | 일회성 스크립트, `source_type=import:dongari`, `moderation_status=approved`(검수 우회) | **조건부 — 확인 대기** | 라이브 공개 API 실측(2 카테고리 121건): officialUrl 전부 빈값(링크 렌더 0), 에브리타임 흔적 0, PII 0, 설명 29~74자 균일(생성 요약 형태). 판정 대기 ① 출처 정체(학교/총동연 공식 vs 민간 aggregator — 민간이면 수집 경계 위반, 사용자 재고지) ② 설명 생성 vs 복사(생성이면 "자동 생성 요약" 표기 또는 검수, 복사면 프로즈 제거). 요구: 스크립트 커밋 또는 handoff에 절차+롤백 쿼리 기록. |
| `27875bd` (백엔드) | L-1 수정: `backend/.gcloudignore`에 `.env`·`.env.*`(`!.env.example`)·`*.pem`·`*.key`·`*-sa*.json`·`var/` 명시 제외 | **PASS** | 기능 검증 `gcloud meta list-files-for-upload backend`: 업로드 대상 176파일 중 `.env` 없음(`.env.example`만), app/ 포함 정상. `#!include:.gitignore` 미사용은 backend/에 .gitignore가 없어 배포 실패를 유발하므로 타당. **L-1 종결 조건 남은 것**: ② 버킷 zip 객체 삭제 ③ 키 로테이션(둘 다 사용자 결정, 백엔드 세션에서 상신) ④ 다음 백엔드 배포 뒤 최신 zip 재검사. |
- **추가 실측(2026-09-08)**: `.env`의 ANTHROPIC_API_KEY와 Secret Manager `ANTHROPIC_API_KEY` v3(enabled)의 SHA-256 일치 → **노출 키 = 라이브 키.** 로테이션 시 새 키 → Secret Manager 새 버전 → 새 Cloud Run 리비전 → 스모크 → 구 버전 disable·Anthropic 측 폐기 → 로컬 .env 갱신 순. RESEND·OPENAI·data.go.kr는 각 콘솔 재발급·폐기. SECRET_KEY(페퍼)는 로컬≠운영이라 유지. 결정은 백엔드 세션에서 사용자 상신 중.
| (판정 갱신) dongari 594건 벌크 적재 | 출처 확인 결과: 개인 GitHub Pages(9yuhyeok.github.io), 원본 url 전부 everytime.kr = **에타 스크랩 산출물의 2차 적재** | **FAIL — 제거 권고(사용자 결정 대기)** | 저작권법 §93 DB제작자권(한 사이트 DB의 상당 부분 594건 재공개, 사실 여부 무관)·부정경쟁방지법 파목·설명문 provenance 미확정(에타 게시글 LLM 요약이면 2차적저작물)·에타 ToS. "이름·분류만" 중간안도 §93 리스크 잔존. 권고: `source_type=="import:dongari"` 일괄 삭제 후 크라우드소싱/총동연 공식 목록으로 대체, 적재 스크립트 미커밋·폐기. 사용자가 유지 시 "리스크 수용" 기록 + "외부 출처 자동요약" 라벨 필수. |
| (결정 기록) dongari 594건 — **사용자 결정 A: 유지, 리스크 수용** (2026-09-08, 백엔드 세션 경유) | 에타 2차 출처(개인 GitHub Pages), 저작권법 §93·부정경쟁방지법 파목 리스크를 사용자가 인지하고 수용 | **사용자 승인 리스크 수용** | 조건 ① 화면에 "외부 출처 · 자동요약" 라벨(프론트 세션 배정, 커밋 시 재게이트) ② 적재 스크립트 미커밋·폐기 ③ handoff에 "2026-09-07 적재 → 유지 결정·라벨 조건" 기록 ④ 롤백 경로 유지(`source_type=="import:dongari"` 일괄 삭제). 보안 권고(제거)는 유지되나 결정권은 사용자에게 있음. |
- **dongari 라벨 확정 문구(2026-09-08, 보안 세션)**: 「학회·동아리 정보 일부는 외부 학생 자료를 바탕으로 자동 정리한 것으로, 각 단체가 직접 제공한 공식 정보가 아니며 정확하지 않을 수 있어요. 잘못된 내용은 신고해 주세요.」 — 출처명 미기재(고지 의무 아님·§93/파목 리스크 회피), "공식 아님"으로 기망 소지 해소, 신고 창구 명시(망법 §44조의2). app/societies 목록 상단, 저대비 숨김 금지. 프론트 커밋 시 문구 일치 여부 재게이트.
| `9298870` (프론트) | dongari 조건 ① 외부 출처·자동요약 고지 라벨(app/societies 목록 상단) | **PASS** | 확정 문구 verbatim 일치, `text-body-sm`·`text-paper-lo` 본문 가독성 + 테두리 박스(저대비 숨김·sr-only·opacity 없음), 목록 위 배치, 신고 기능 연결. 단독 커밋(revert 용이). **종결 조건**: 배포 뒤 라이브 /societies에서 문구 노출 실측. |
| `c33d451` (프론트) | /explore 마인드맵 허브 + 학우검색 /explore/people 이동 | **PASS** | 허브 링크 3개 정적 내부 경로(/explore/people·/certifications·/societies), 프로필 링크는 서버 uid 기반 내부 경로(기존 패턴), `VerifyGate` 인증 게이트 보존, HTML·스토리지 싱크·외부 URL 0. 순수 내비. |
- **dongari 조건 ① 종결 (2026-09-08, 라이브 실측)**: 프론트 리비전 `ourlab-frontend-00035-67l`(9298870 라벨 + c33d451 허브). `/societies` 200·확정 문구 1회 노출, `/explore` 200·허브 3노드 노출, 런타임 SA `ourlab-frontend-runtime@` 유지. → dongari 유지 조건 ①~④ **전부 충족**(리스크 수용 기록 유지). 보안 헤더는 H-1 보류대로 미적용(`x-powered-by` 잔존).
- **범위 정정(2026-09-08, 백엔드 지적 → 실측)**: 앞서 "커밋 전 업로드 3건은 .gitignore 적용"이라 한 판단은 **오류**. gcloud는 소스 디렉터리 자체에 `.gitignore`가 있을 때만 자동 제외를 만들며 `backend/`에는 없었다 → `318b143` 이전 업로드는 **아무것도 제외되지 않음**(아카이브 173~181MB, `.venv` 14,900·`node_modules` 1,952 항목 포함). 표본 `…9f52cdb3…zip`(2026-09-07 12:06 KST) 내부에 `.env` 확인. 따라서 **노출 창 = 첫 Cloud Run 소스 배포(2026-08-31)~2026-09-07, 백엔드 아카이브 24개(3.38GiB) 전부 `.env` 포함**으로 본다. 키 로테이션 범위는 동일(Anthropic 라이브키·Resend·OpenAI·data.go.kr; 08-31 이전 아카이브엔 이미 disable된 구 Anthropic 키도 포함 가능). **B 권고 갱신: 24개 전체 삭제**(빌드 완료된 불활성 아카이브, 서비스 영향 0).
| `d452ba7` (프론트) | SideRail 중복 버튼 제거 + '← 탐색' 내부 링크 | **PASS** | 추가된 링크는 정적 `/explore` 2개뿐, 싱크·폼·스토리지·외부 URL 0. 배포 후 /societies 고지 문구 잔존 재확인 예정. |
- **B 종결(2026-09-08)**: 사용자 승인("지워") 후 백엔드 세션이 `services/ourlab-backend/*.zip` 24개(3,626,582,164 bytes, 2026-08-31~09-07) 삭제, 삭제 전 목록 캡처는 백엔드 세션 기록. 보안 세션 독립 실측: 잔여 아카이브 **0개**. 서빙 리비전 불변(backend 00031-9l7 / frontend 00035-67l), /health 200. 프론트 버킷 아카이브는 비밀값 아님(NEXT_PUBLIC)이라 별도 정리 항목. **L-1 남은 것: C(키 로테이션, 사용자 결정) · ④(다음 백엔드 배포 시 최신 zip 재검사).**
- **프론트 버킷(2026-09-08, 백엔드 확인)**: `frontend/.gitignore`가 존재(`.env*.local` 제외)해 `.gcloudignore` 없이도 gcloud 자동 제외 적용 — 아카이브 36개 전부 ~750KB, `.env.local`·`.env.production.local` 미포함(설령 포함돼도 NEXT_PUBLIC 공개값뿐). **조치 불요.** 백엔드에 `.gitignore`가 없었던 것이 L-1의 근본 원인이었음을 재확인.
- **C 결정(2026-09-08, 사용자, 백엔드 세션 경유)**: 키 로테이션 **보류 → 실사용자 런칭 직전 실행**. 원문 "교체는 나중에 실제 유저들한테 배포할 때 하자. 백로그에 적어놔". 범위·절차는 `docs/BACKLOG.md`(백엔드 기록): Anthropic 라이브키(=유출키)·Resend·OpenAI 폐기·data.go.kr 재발급, SECRET_KEY 불변; 새 키→Secret Manager 새 버전→새 리비전→스모크→구 버전 disable→로컬 .env. **보안 세션 종결 절차(런칭 시)**: 리비전명 수신 → SM 활성 버전·리비전·최신 소스 zip(④) 한 번에 실측. 그때까지 L-1 상태: 코드 수정 완료·아카이브 삭제 완료·**키는 유효한 채 보관함 밖 노출 없음(프로젝트 내부 노출만 있었음)** — 리스크 수용은 사용자 결정.
