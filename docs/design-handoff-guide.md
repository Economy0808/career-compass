...
2. **더블클릭(달성 토글)은 네이티브 dblclick이 아니라 `handlePointerUp`의 수동 페어링이다**
   (2026-09-02 — OS의 좁은 더블클릭 허용 사각형(Windows 기본 4px) 탓에 미세 지터만으로
   dblclick 이벤트가 안 만들어져 간헐 실패했다; 자체 판정은 400ms/12px로 관대하다).
   pointerdown은 노드 `<g>` 전체(r=22)에서 시작하므로 판정 영역 규칙은 그대로다 —
   자식 원으로 내리면 좁아져 "달성이 안 된다"가 재발한다. **네이티브 onDoubleClick을
   노드에 다시 달지 말 것** — 수동 판정과 겹쳐 토글이 두 번 실행(=원상복구)된다.
...
9. **패널은 그래프 위에 떠 있는 오버레이**다(전체화면 캔버스가 바닥). 그리드 컬럼으로 바꾸면
   좌표 변환이 어긋난다.
10. **`--ink-900` 위에 그리는 선에 `--rule`을 쓰지 말 것.** `--rule`(#2a3350)은 패널·칩처럼
    밝은 면 위의 계선용이고, 캔버스 바닥 위에서는 대비가 **1.4:1**이라 사실상 보이지 않는다
    (비텍스트 최소 기준 3:1). 캔버스 위의 선은 `--rule-edge`를 쓴다 — opacity 0.7에서 3.6:1.
11. **선이 안 보인다고 굵기를 올리지 말 것.** 균일하게 굵어진 실선은 성도가 아니라 "볼펜으로
    그은 것"이 된다(사용자 명시 금지 조건). 가시성은 **명도**로 올리고 굵기는 미점등 1px /
    점등 2px를 유지한다.
12. **미달성 노드를 흐리게 해서 달성을 표현하지 말 것.** 달성의 신호는 밝기 차가 아니라
    **백색 고온핵 + 헤일로 확장 + 회절침 신장 + 에어리 링**이다. 미달성도 그 자체로 또렷한
    별이어야 한다(opacity 0.88~0.95) — 흐리게 깔면 "달성 전에 노드가 너무 희미하다"가 재발한다.
13. **유형 = 형태, 색 = 사용자** (2026-09-02 별상 Rev.B, 사용자 승인). 노드의 유형은 색이
    아니라 **별상(星像) 형태**가 말한다 — 색이 비슷해도·색약이어도 유형이 읽히게. 색은
    노드별 자유 RGB(팔레트·커스텀 피커·스포이드)이고 분광형 5색은 **기본 추천값으로 강등**됐다
    (제거는 금지 — 유형 기본색과 GLOW_PRESETS 계보가 걸려 있다). 형태 매핑(전부 실제 망원경
    광학 현상, `components/StarBody.tsx`가 단일 소유 — 캔버스 노드와 LLM 시안 다이브인 멤버가
    함께 쓴다):

    | 유형 | 형태 | 근거 |
    |---|---|---|
    | 수업 course | 십자 회절 (달성 시 45° 부침) | 반사경 지지대의 기본 별상 |
    | 자격증 certification | 육각 회절 (+수평 부침) | 육각 거울(JWST)의 별상 |
    | 학회 organization | 쌍성 (주핵>부핵, 한 헤일로 공유) | 함께 도는 사람들 |
    | 대외활동 activity | 유성 (-45° 이중 꼬리) | 밖으로 나간 움직임 |
    | 네트워킹 networking | 산개성단 (주성+위성별 3) | 사람에서 사람으로 |

    구현 규칙: 그라디언트는 **노드마다가 아니라 색마다** 한 벌(`StarColorDefs`), 스톱에
    실제 hex를 굽는다(currentColor는 그라디언트 정의 위치 기준으로 풀리는 SVG 함정).
    블러는 공유 `#const-glow` 하나를 **달성 별에만** 래핑 — 별마다 필터를 만들지 말 것
    (노드 수백 개 성능). 형태를 바꾸려면 `StarBody`와 이 표를 같이 고칠 것.
