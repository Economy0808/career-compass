...
> **30차 (9/7, Phase 3 코드 완료)**: 로더+검색 API 구현. `scripts/load_curated_certifications.py`(reviewed=true 게이트, 기본 dry-run, `--commit` 필요-이번 세션 미실행), 국가자격은 종목목록(DATA_GO_KR_API_KEY) name_norm 매칭→시험일정(DATA_GO_KR_SERVICE_KEY) 보강, career_paths 컬렉션 로드. `GET /api/certifications?q=&scope=`·`/by-career`. **버그 수정**: 시험일정 JSON을 실제 header/body 형태로 파싱, numOfRows 100→50(>50이면 API가 resultCode 930으로 조용히 0건-잠복버그), refresh_certifications 키 배선(종목목록=api_key/시험일정=service_key). **드라이런 실측**: 종목목록 613, 2026일정 157건. 국가자격 79 중 **43 매칭/36 미매칭**(대부분 타부처 면허라 Q-Net에 없음=정상: 의사·간호사·사회복지사·정사서 등; 별칭갭 ~5: 경영지도사(마케팅부문)→(마케팅), 바이오화학제품제조기사(구…) 괄호, 청소년상담사·메카트로닉스기사·식품기사는 급수/명칭 차이). 미매칭은 큐레이션 official_url 유지. 테스트 45통과, Firestore 미기록. .env 키 2개 실검증 완료(둘 다 정상). **다음**: ①별칭갭 소폭 보정 ②실제 적재(격리 에뮬레이터 `--commit`→export→사용자 승인→프로덕션) ③유저추가(제출·검토, 학회 패턴) ④프론트 검색/추가 UI(피어). 재게이트 대상(보안): 로더 reviewed 게이트 + etl/refresh 버그수정 diff.
>
> **29차 (9/7, 자격증 큐레이션 완결)**: 사용자 검수 완료 - 85 후보 중 유용11+민간2(병원행정사·보험심사평가사) 컷, 나머지 확정. `certifications_curated.json` reviewed=true, **총 100개**(28 파일럿 + 72 신규; 국가79/민간12/국외9, 필수43/우대53/유용4), career_cert_map 90진로, cert-light 26진로. **자격증 트랙 데이터 완결.** 미처리 학과 ~100개(인문·순수과학·예체능·법·교양 등 자격증 희박)는 스킵 유지. **다음=Phase 3(미착수)**: 로더(certifications_curated.json→certifications 컬렉션, reviewed=true 게이트, sources.yml 게이트, 격리 에뮬레이터→export→사용자 승인→프로덕션, 국가자격은 refresh_certifications로 Q-Net 공식URL·시험일정 보강[신규 DATA_GO_KR_SERVICE_KEY 필요]) + career_paths 컬렉션 로더 + GET /api/certifications 검색 엔드포인트 + 유저추가(제출·검토, 학회 패턴) + 프론트 검색/추가 UI(피어).
>
> **28차 (9/7 밤, 자율) - 나머지 계열 자격증 전수 조사, 85개 검수 대기**: 사용자 취침 지시로 자율 실행. 공학(39)·보건바이오(22) 이미추론분 + 의약보건(간호·물치·방사선·임병·작치·치위생·의·치의·영양·위생 = 국가면허 중심 14진로)·사회교육(사회복지·심리·사서·보육·평생교육·체육·한국어교원·실내건축 12진로) 신규 정의. 8개 서브에이전트(WebSearch, 링커리어 차단). **병합·중복통합(면허/급수 통일, 기사/산업기사/기술사 유지) → 새 자격증 85개**(국가79/민간6, 필수35/우대39/유용11) `_pending_cert_review.md`·`_pending_cert_merged.json`. 정직하게 cert-light 진로 19개 표시. 커밋: cert_candidates_{공학1-4,바이오1-2,의약보건,사회교육}.json + pending + _taxonomy_dump. **미처리(자격증 희박)**: 159학과 중 파일럿25+cert-dense22 처리, 나머지 ~100개(인문·순수과학·예체능·법·교양·영문중복·캠퍼스융합)는 자격증 거의 없어 스킵 - 사용자 결정 대기(`_taxonomy_dump.json`). 서브에이전트가 폐지·오표기 자격증 다수 교정(반도체설계기사 폐지, 생물공학기사→바이오화학제품제조기사, 지반공학기술사→토질및기초기술사, 화장품제조관리사→맞춤형화장품조제관리사). **다음=검수 후 확정→Phase 3 로더(28+확정분 DB 적재, reviewed=true만, 격리 에뮬레이터→승인→프로덕션, 국가자격 Q-Net 보강)+검색+유저추가.**
>
...
> - **⚠️인덱스 누락은 로컬에서 영원히 안 드러난다 — 같은 함정에 하루 두 번 걸렸다.** 에뮬레이터는
>   복합 인덱스를 강제하지 않으므로, `firestore.indexes.json`에 선언이 **아예 빠져 있어도** 로컬
>   테스트·QA를 전부 통과한다. 운영에서만 `FailedPrecondition: The query requires an index`로 500이
>   난다. ①커뮤니티·DM은 선언은 있었고 빌드 대기였는데, ②`stories`는 **선언 자체가 없었다**
>   (`story_repo._active_query`가 `owner_id ==` + `expires_at >` 두 조건을 건다). `a38f49d`로 추가.
>   **대응**: 리포 전체(`app/firestore/*_repo.py`)를 훑어 복합 인덱스가 필요한 쿼리를 뽑고 선언과
>   대조하는 감사를 돌렸다 — 누락은 `stories` 하나였다. 이론에만 기대지 않으려고 인증 상태로
>   **18개 엔드포인트를 전수 호출해 18/18 200**까지 실측했다. 새 쿼리를 추가할 때는 조건이 2개
>   이상이거나 `where` + 다른 필드 `order_by`면 **선언을 먼저 추가**할 것.
> - **⚠️프론트를 고쳤는데 화면이 안 변하면, 프론트를 더 파기 전에 그 API를 직접 호출해 상태 코드를
>   봐라.** 프로필 스토리 링이 그랬다 — 프론트 세션의 인증 게이트 수정은 옳았는데 백엔드가 500을
>   주고 있었다(위 인덱스 누락). 화면만 보고 프론트를 더 뜯었으면 헛수고였다. CORS `expose_headers`
>   건(`de518aa`)과 같은 계열이다: **도구가 보여주는 것과 실제가 다른 부류.**
> - **라이브 판정은 브라우저 콘솔이 아니라 서버 로그가 권위다**(위 CORS 항목과 별개로 재확인).
>   프론트 401 수정 배포 후 콘솔엔 옛 401이 그대로 남아 있었다 — 도구의 콘솔 버퍼가 네비게이션을
>   넘어 이전 항목을 들고 있던 것. `gcloud logging read ... httpRequest.status=401 --freshness=5m`
>   으로 갈랐고 실제로는 0건이었다.
