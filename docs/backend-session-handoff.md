...
7. **붙임 1 PDF(학칙·규정류) 용도 미확정** — 사용자가
   `C:\Users\user\Downloads\[붙임 1] 2026학년도 연세대학교 대학요람(학교소개, 학칙 및 규정류)_202605.pdf`
   (37p)를 업로드했으나 지시가 중단되어 용도가 정해지지 않았다. 유력 후보: 군집별 AI 조언(C 항목)이
   학사 규정(전과 요건, 복수전공 신청 자격 등)을 근거로 말하게 하는 참조 데이터. 파싱 전에 사용자
   확인 필요. 참고: 붙임 2·3은 이미 txt 변환본으로 파싱 완료(과목 7,109개), pdfplumber는
   pyproject.toml에 선언돼 있음.
