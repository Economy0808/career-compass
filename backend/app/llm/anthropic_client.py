...
class AnthropicClaudeClient:
    ...
    async def select_relevant_departments(
        ...
        system += (
            " 목표 직업에 직접 대응하는 학부 전공이 없으면(예: 법조인 — 학부 법학과"
            " 부재) 그 진로의 기반이 되는 인접 학과들을 선택하라(예: 정치외교·행정·"
            "철학·사회 등). 확신이 없어도 기반 학과 1~2개는 선택하는 편이 빈 선택보다"
            " 낫다."
        )
        system += (
    ...
    async def cluster_courses(
        ...
        system = (
            ...
            # 아니라 쪼개는 것이므로 위의 "빠짐없이" 원칙과 충돌하지 않고, 선수과목
            # 추론 입력 상한(_MAX_PREREQ_ITEMS=50)과의 정합도 함께 지켜진다.
            "- 한 군집은 6~12과목이 적당하고, 15과목을 절대 넘기지 마라. 담을 과목이"
            " 그보다 많으면 버리지 말고 더 구체적인 하위 주제로 군집을 쪼개라."
            " 쪼개는 것은 담는 방식의 문제일 뿐이니 총 과목 수를 줄이지 마라.\n"
            # 형제 군집 이름 규약(2026-09-02 사용자 C안 확정: "일단은 C로 진행해").
            # 프론트(DraftReviewStage)가 ' — '(공백-대시-공백)를 구분자로 파싱해
            # 같은 상위 주제의 형제 성운을 인접 배치·묶음 표시한다. 구분자를
            # ' — '로 둔 이유: 과목·군집 이름에 흔한 가운뎃점('재무·회계')과
            # 절대 충돌하지 않아야 해서다. 구분자를 바꾸면 프론트 파싱과 함께
            # 바꿔야 한다.
            "- 한 주제를 여러 군집으로 쪼갤 때는 이름을 '상위 주제 — 하위 주제'"
            " 형식으로 지어라(구분자는 공백을 두른 대시 ' — ' 하나만). 예:"
            " '수학·과학 기초 — 미적분·해석학', '수학·과학 기초 — 선형대수',"
            " '수학·과학 기초 — 일반물리'. 쪼개지 않은 단독 군집 이름에는 이"
            " 구분자를 쓰지 마라. 같은 상위 주제의 형제 군집들은 배열에서 서로"
            " 붙여 연속으로 나열하고, 형제들의 상위 주제 표기는 글자 하나까지"
            " 동일하게 써라 — '전자공학'과 '전자공학 기초'처럼 비슷하지만 다른"
            " 표기를 섞으면 화면에서 한 묶음으로 모이지 않는다(실측된 실패"
            " 사례다).\n"
    ...
    async def suggest_support_elements(
        ...
        system = (
            ...
            "- 전문직 라이선스(변호사·공인회계사·세무사 등)는 그 자체가 평생 직업이므로"
            " 목표의 대표 추천으로 단독 제시하지 마라. 보조·병기로만 다루고, 목표와는"
            " 별개의 진로임을 description에 명시하라."
        )
        system_blocks = _cached_system(system)
        if rules_context:
            system_blocks.append(
