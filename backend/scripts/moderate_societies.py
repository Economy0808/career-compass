...
Usage (backend/ 에서, .venv 활성화 후):
    ...
    python scripts/moderate_societies.py approve <category> <id>
    python scripts/moderate_societies.py reject <category> <id>
...
SocietyCategory 14개 분야 enum). <category>는 그 enum 문자열 그대로 넣는다.
...
def _cmd_list_pending(db) -> None:
    ...
    for item in pending:
        ...
        print(
            f"{item['category']}/{item['id']}  {item['kind']}  {item['name']}  "
            f"{item['official_url']}  (submitted_by={item['submitter_uid']}){flag}"
        )


def _cmd_moderate(db, category: str, doc_id: str, status: str) -> None:
    found = society_repo.set_moderation_status(db, category=category, doc_id=doc_id, status=status)
    if not found:
        print(f"에러: {category}/{doc_id} 문서를 찾을 수 없습니다.")
        raise SystemExit(1)
    print(f"{category}/{doc_id} -> {status}")
...
def main() -> None:
    ...
    sub.add_parser("list-pending", help="전체 분야의 대기중(pending) 제출 나열")

    approve_p = sub.add_parser("approve", help="제출을 승인 처리")
    approve_p.add_argument("category")
    approve_p.add_argument("id")

    reject_p = sub.add_parser("reject", help="제출을 거절 처리")
    reject_p.add_argument("category")
    ...
    _cmd_moderate(db, args.category, args.id, status)
