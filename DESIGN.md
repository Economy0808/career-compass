...
배경 등 **관측 표면 자체**는 여전히 ink 전용이고, 그 위에 얹히는 판만
...
**모달은 단일 규칙이 아니다 (2026-08-28 개정).** 모달을 무조건 ink로 두면
안 된다 — 성격에 따라 갈린다. **섬 성격 모달**(떠 있는 크롬 자체의 확장,
예: `LaunchModal`의 "별자리 띄우기" 창)은 그 크롬과 같은 종이 —
`.paper-surface` + `islandExpand`를 그대로 쓴다. 반대로 **관측 컨텍스트
모달**(제목 입력, 노트 전체화면 편집기 등 캔버스 위에서 진행하는 작업창)은
관측 표면의 연장이므로 ink로 남는다. 새 모달을 추가할 때는 "이게 떠 있는
크롬이 잠깐 커진 것인가, 캔버스 작업의 연속인가"를 먼저 판정한다.
